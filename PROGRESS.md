...
- **Two real bugs found and fixed, not just one.** Passing a JS array into a drizzle `sql` template with `${array}::uuid[]` does **not** bind it as a single Postgres array parameter — drizzle expands each element into its own scalar placeholder, so `any($array::uuid[])` receives a bare `(a,b,c)` ROW constructor and Postgres rejects the cast ("cannot cast type record to uuid[]"). Found building this slice's own market-basket query; fixed with `sql.join` + an explicit `array[...]` constructor (the same pattern this codebase already uses for `IN`-lists — apps/captain's `fireOrder`, apps/kds's board queries). **The identical bug was already sitting in Slice 2's shipped `booth-host.ts`**, in the `avoidAllergens` filter — never caught because no live test ever actually selected an allergen chip. Fixed the same way, same commit.
- Verified live end-to-end on both surfaces against the real seeded Ember & Oak data (real market-basket signal — e.g. "Bourbon BBQ Burger" co-occurring with "Chocolate Lava Cake" in 167 real orders): Booth's cart rail renders real varied fallback reasons and its Add button correctly adds the suggestion to the cart; Captain's order-screen strip renders identically (same numbers, same reasons) and its Add button reuses the existing `addOrderItem` action. `pnpm -w typecheck && pnpm -w lint` green across all 12 packages.
- A transient Turbopack dev-overlay "N Issues" badge appeared during on-demand route compilation on Captain's first visit after a server restart — chased down and confirmed to self-clear a moment later with zero server-side errors and zero browser console errors at any point; not a real bug, noted here so a future session doesn't re-chase it.
- **Live QR codes for Ember & Oak's 8 tables were re-minted** (`packages/db/qr-codes/BW1/T1.png`–`T8.png`) after local verification testing overwrote the on-disk PNGs with `localhost`-pointing ones — the tokens given out earlier the same day are now revoked; the current live links are in this session's own chat history, not repeated here (they're a live secret, same as any raw token).
- **Next up: Slice 4, Review→Action** (§5B) — structured aspect/sentiment/dish extraction from `feedback` + pasted aggregator reviews, per-dish sentiment trend + "3 things to fix this week" on Console. No review data source exists yet for Ember & Oak (Slice 0 flagged this) — check what's available before starting.
...
**Phase 6 — AI Layer v1:**
- **Slice 0 (real-data importer) is done.** The owner provided a real steakhouse POS export — small, US-priced, no order/session grouping, no tax data, wrong market for this product's domain. Re-branded to an Indian premium grill ("Ember & Oak," Mumbai — Bandra West, its own new org/brand/outlet — `packages/db/scripts/import-steakhouse.ts` + `data/steakhouse-fixture-ids.ts` + `data/steakhouse-menu.ts`) per the owner's explicit call, and built as a **seeded, deterministic generator** reproducing the source data's real shape (items, price spread, category/payment/channel/day-of-week/hour patterns) rather than a literal transcription — the pasted file was too large to mechanically retype without real risk of truncation/errors. Full reasoning, including the two `AskUserQuestion` rounds this required: [DECISIONS.md](DECISIONS.md).
- Verified on both local DBs: 1,424 orders / 4,629 order items / 2,999 KOTs / 1,424 settled bills / 1,424 captured payments, zero money-invariant violations, `Σ payments == Σ bills.payable` exactly, invoice numbers gapless (`A1/2627/000001`–`001424`), and **RLS isolation confirmed against real Postgres** — an existing believable-chain-fixture staff member sees 0 rows from this new, unrelated outlet.
- Two real things found by actually running it, not just writing it: `@restrobooth/domain` isn't consumable from `packages/db`'s stricter module resolution (worked around the same way `seed-believable-chain.ts` already does, for the same reason — real follow-up: give domain an actual build step); and the partitioned tables only cover a rolling window around real `now()`, so the generator's date range had to be relative to today, not the source file's own 2024 labels.
...
- **Slice 2 (Booth Host, §5A) is done.** Migration `0033_menu_item_spice_tags.sql` adds `menu_items.spice_level`/`.tags` (hand-tagged, real culinary judgment, 165/165 items matched by name); `menu_items.embedding` backfilled with real `Supabase/gte-small` vectors via a Node script (`packages/db/scripts/backfill-embeddings.ts`, `@huggingface/transformers` — the owner's explicit call over standing up a Supabase Edge Function neither this project nor Supabase-local actually has). `apps/booth/lib/booth-host.ts` is the shortlist: diet + allergens are **hard** SQL `WHERE` filters (dietary safety, never relaxed), spice/mood/budget-band fit are **soft** ranking-score signals scored alongside real historical popularity (`order_items` grouped by dish) — the LLM never picks a dish, it only writes the per-dish reason string, reusing Slice 1's `checkBudget`/cache/timeout wrapper wholesale. Budget bands are per-store price tertiles (`percentile_disc`), not fixed rupee thresholds — Spice Route and Ember & Oak have wildly different price ranges. AI-off fallback (`fallbackReason()`) generates real, differentiated reason strings from the same match flags, not a placeholder. `BoothHostIntake.tsx` (chip-based, one-tap Skip, never blocks menu render) + `PickedForYouRail.tsx` (horizontal-scroll rail with Add-to-cart) are the guest-facing pieces — screenshotted and verified live end-to-end against a real seeded table, AI-off path confirmed (no `OPENROUTER_API_KEY` in `apps/booth/.env.local`), matching ADR-0007 §3's "turn the AI off, everything still works" acceptance bar.
- **A real, non-trivial build bug found and fixed**: `@restrobooth/ai` shipped with `"main": "./src/index.ts"` (raw source, no build step) like `@restrobooth/domain` — but unlike domain, this broke immediately the first time any Next app (`apps/booth`) actually imported it, because `@restrobooth/db` (which `@restrobooth/ai` imports) only resolves cleanly under Turbopack because **it already has a real `dist/` build** (`"main": "./dist/index.js"`) — nothing about the raw-source pattern was actually working generically, `@restrobooth/db` was just never being consumed as raw source by a bundler to begin with. Fixed by giving `@restrobooth/ai` the exact same build step `@restrobooth/db` already has (`tsconfig.build.json`, `pnpm build` → `dist/`), and adding it to `apps/booth/next.config.ts`'s `transpilePackages`. **`@restrobooth/domain`'s own module-resolution gap (flagged in Slice 0) is the same category of bug** — worth fixing the same way before the next package tries to consume it from a bundler context, not just from `packages/db`'s scripts.
- **Next up: Slice 3, Smart upsell** (§5E) — market-basket lift computed in SQL, LLM writes the one-liner, same "numbers are never AI" shape as the Booth Host.

### Local dev note
If a future session sees `packages/channels` still an empty stub, `packages/ai` filled in, an "Ember & Oak" outlet nobody remembers seeding, and CLAUDE.md's gate section reading like a status note rather than a hard rule — that's all correct as of 2026-07-23, check DECISIONS.md before assuming something regressed. The OpenRouter API key used for the live smoke test is NOT stored in any tracked file — get it from the owner again if `packages/ai/src/openRouterProvider.live.test.ts` needs to be re-run. The steakhouse import is additive and safe to re-run (`pnpm --filter @restrobooth/db import:steakhouse`) — it uses fixed IDs, so re-running against a DB that already has it will hit unique-constraint errors; clean up that outlet's rows first if you need a fresh copy (see DECISIONS.md's delete list, reverse FK order).
