...
/**
 * The POS's single door to Postgres. Same pooling/caching discipline as
 * apps/console/lib/db.ts — a fresh pool per call would exhaust Postgres
...
function getDb(): Database {
  if (!globalForDb.__restroboothDb) {
    ...
    if (!url) throw new Error("DATABASE_URL is not set (apps/pos/.env.local).");
 ...
 * (and the Phase 3a capability layer — drizzle/0014) applied. Throws if
 * there is no session; every POS route is behind auth, so a missing
 * session here means the caller is a route the middleware doesn't cover,
 * which is a bug, not a normal path.
 */
