...
> **Amendment 4 — 2026-07-20, floor-card notification band.** The waiter-called badge/button pair (Amendment 2's territory) was a variable-height footer — present only when there was something to say, which meant one alerted table could be taller than its grid neighbours and, because CSS grid stretches every card in a row to the tallest, visibly stretch its whole row. Owner feedback across two passes (first "the Acknowledge button goes out of the card," then "the table having waiter requested is taller than other cards") converged on a structural fix, not another CSS patch.
> - **Universal reserved footer.** Every table card (POS `FloorMap.module.css`'s `.notifyBand`, Captain `FloorList.module.css`'s equivalent) now ends in the SAME fixed-height strip (48px) regardless of state — idle, self-seated, billed, or waiter-called. Content is priority-ordered and mutually exclusive (waiter call > bill status > self-seated tag > empty), never stacked, so the card's total height never changes and the grid never jumps. This is also the extension point for future service signals (payment requested, food ready, kitchen delay) — one more priority branch, not a new layout. **No UI was built for those yet** — they aren't real columns (CLAUDE.md: don't build against invented data).
> - **De-escalated card border.** The full-saturation `border-color: var(--signal-600)` ring around a waiter-called card competed with the ramp chip's own critical-red for attention — three red signals stacked (chip, card ring, badge) for one event. Softened to a `color-mix` tint (30%) plus a subtle tinted shadow; the notification band itself now carries the alert's colour and copy, so the card border is a quiet lift, not a second alarm.
> - **A second motion exception, same scoping discipline as Amendment 2.** Within `.floorMotionScope` only (POS's floor grid — Captain was NOT extended, it has no equivalent scope and stays fully static per its existing zero-motion rule), gated by `@media (prefers-reduced-motion: no-preference)` and using `!important` to out-specify `tokens/motion.css`'s blanket kill-switch (the same mechanism Amendment 2's transitions use): the notification band's icon pulses (opacity, 3s loop) when critical, and the band's swapped-in content slides up on mount (200ms, plays automatically since React unmounts/remounts the child on every state change). The card and the band's own height never animate — only the icon and the content within the already-fixed slot.

> **Amendment 5 — 2026-07-21, "Cooking" notify-band state.** Requested directly by the owner: when a table has a KOT still in flight (any status short of bumped/voided — `apps/pos/app/floor/queries.ts`'s new `hasActiveKot`, a plain `EXISTS`, same shape as `hasPendingGuestPayment`), the notify band shows "Cooking" instead of falling through to the self-seated tag or empty slot. Slotted into the existing priority chain below bill status (once a bill's in motion that's the more current story for the table, even if a late add-on KOT is technically still active) and above the self-seated tag (a static context badge, a live kitchen status should outrank it): waiter call > payment to confirm > bill status > **cooking** > self-seated > empty.
> - **Tone: a new `active` value, not warning/critical.** Cooking is ongoing, not alarming — reusing amber/red would misread it as needing attention. `--enamel-500` (the token table's own "Live / fresh / OK" brand accent, `colors.css`) tints the band; `--enamel-700` (the AA-text-safe variant) colours the label, same fill/text split the rest of the palette already follows.
> - **A third motion exception, same scoping and mechanism as Amendment 4** — `.floorMotionScope` only, `@media (prefers-reduced-motion: no-preference)`, `!important` over the blanket kill-switch: a hand-drawn 🍲 rotates gently (±8°, 1.2s loop) instead of sitting static. Nothing else about the band — its layout, height, or the rest of the priority chain — changed.
> - **Booth's guest-facing `OrderStatusBoard.tsx`** already labelled a fired item "Cooking" (guest-facing presentation, not a business rule — see that file's own comment); this amendment only added the same 🍲 next to that existing label, animated via `framer-motion` since Booth is the one app with full, ungated motion allowance (`useMotionAllowed()`), not a new exception there.
