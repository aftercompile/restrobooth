...
Whether even the hoisted version holds at 20 outlets / 9M rows is an open question, not an assumption. It is **BENCH-01** in [BENCHMARKS.md](BENCHMARKS.md), and the fallback (caching the accessible-outlet ID array in the JWT `app_metadata`, refreshed on membership change) is specified there. See [adr/0006-override-resolution.md](adr/0006-override-resolution.md) for the analogous decision on the menu side.
